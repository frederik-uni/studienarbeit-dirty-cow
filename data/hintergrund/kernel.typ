...
== 2.2.4 Scheduler und Speicherverwaltung
...
=== 2.2.4.1 Scheduler
Die Rolle des Scheduler im Linux-Kernel ist die Aufteilung der CPU-Zeit auf Prozesse @scheduler. Derartige Prozesse befinden sich üblicherweise in den Zuständen runnable (lauffähig/wartend auf CPU), running (laufend) und blocked/sleeping (wartend auf Speicherergebnisse).
...
== 2.2.6 Pagecache Pages
Die Funktion des Page Cache im Kernel ist eine Steigerung der Effizienz, indem er Dateiinhalte im RAM zwischenspeichert und somit ineffiziente Zugriffe auf Hard Disk Drive (HDD) oder Solid State Drive (SSD) minimiert. Im Bezug darauf stellt eine Pagecache Page eine Speicherseite im RAM dar, die einen speziellen Abschnitt einer Datei repräsentiert.

Während des Lesevorgangs findet eine Kernel-seitige Prüfung des Page Caches statt. Insofern die Dateidaten bereits im RAM gespeichert sind wird von einem Cache Hit gesprochen und der Zugriff erfolgt sehr schnell, falls diese fehlen wird von einem Cache Miss gesprochen und ein Nachladen der Daten von dem Datenträger ist erforderlich um sie als neue Pagecache Page abzulegen.  
Im Folgenden besonders relevant ist der Page Cache bei der Funktion ``mmap() @relevant, da file-backed Speicherabbildungen üblicherweise auf Pagecache Pages verweisen und somit eine Vielzahl an Prozessen denselben Datenbereich parallel nutzen können. 
...
== 2.2.8 Was ist eine Pseudo Datei
